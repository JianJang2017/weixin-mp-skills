...
仅在选择公众号视觉风格时读取本文件。选定风格后，再加载对应的主题文件。
...
- 如果用户没有指定，按视觉目标选择：清爽结构选冷感蓝图；安静慢读选极简墨色；稳重可信选暖白秩序；清新编辑感选森绿编辑；纸媒叙事选复古杂志；紧凑摘要选蓝灰简报；深色仪式感选黑金舞台；亲近手账感选手账便签。
...
每个风格都有一个可直接在浏览器打开的 HTML 用例，位于 `references/examples/`。打开 `references/examples/index.html` 可以进入总览页；单个用例只用于预览排版效果，不作为发布内容模板。
