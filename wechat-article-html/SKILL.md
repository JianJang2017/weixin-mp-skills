...
description: 为微信公众号创建可直接粘贴的图文 HTML，包含内联样式、可选主题、可发布的 HTTPS 图片地址，并默认避免正文超链接标签。当 Codex 需要把 Markdown、纯文本、草稿、大纲或长文转换成可在浏览器打开、复制并粘贴到微信公众号编辑器的富文本 HTML 时使用；支持将链接改写为“参考文章/参考链接”可见文本块、本地预览图片、远程图片托管，以及 references/ 中的多套主题模板。
---
...
3. 构建 HTML：
   ...
   - 主要使用 `section`、`p`、`h1`、`h2`、`img` 和 `strong` 标签。
   ...
   - 代码块不要依赖 `<pre>`、`<code>` 或 `white-space`。用带内联样式的 `<p>`/`section` 承载代码内容；代码文本必须转义，换行写成 `<br>`，缩进和连续空格写成 `&nbsp;`。
4. 处理图片：
...
5. 完成前校验：
   ...
   - 快速检查 HTML，避免未转义的 `<script`、`onclick`、`class=`、`<a href`、`<pre`、外部样式表或损坏的相对路径。
...
- 将代码块和长 URL 视为手机端溢出风险。代码块使用显式 `<br>` 和 `&nbsp;` 保留换行/缩进，并搭配 `word-break:break-all`、`overflow-wrap:anywhere`；不要依赖 `white-space:pre-wrap`。
