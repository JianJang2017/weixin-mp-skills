...
2. 发布脚本会在上传前自动把 `<pre>` 代码块归一化为微信公众号更稳定的 `<p>` + `<br>` + `&nbsp;` 结构，避免草稿中丢失换行和缩进。
3. 在任何真实 API 调用前确认必需字段：`title`、HTML `content`，以及封面图片路径或 `thumb_media_id`。
4. 先运行 `scripts/wechat_mp_publish.py draft --dry-run` 检查载荷并捕获缺失字段；含代码块的文章还要确认 dry-run payload 的正文中没有 `<pre`，并且保留了 `<br>` 和 `&nbsp;`。
5. 使用同一命令去掉 `--dry-run` 创建草稿。
6. 只有在用户明确确认后才发布，命令为 `scripts/wechat_mp_publish.py publish --media-id MEDIA_ID --confirm-publish`。
7. 使用 `scripts/wechat_mp_publish.py status --publish-id PUBLISH_ID` 查询发布状态。
